#!/usr/bin/env python3
"""Build a monthly CHL mosaic Zarr from the raw CHL mosaic Zarr."""
...
LOGGER = logging.getLogger("chl_mosaic_to_monthly_zarr")

DEFAULT_INPUT = Path(
    "/home/pabrojast/Proyectos/stacprocess-ihp/data/terrascope_all_grids_full_history_zarr/chl_mosaic_ts.zarr"
)
DEFAULT_OUTPUT = Path(
    "/home/pabrojast/Proyectos/stacprocess-ihp/data/terrascope_all_grids_full_history_zarr/chl_mosaic_monthly.zarr"
)
DEFAULT_VARIABLE = "CHL"
DEFAULT_FREQ = "MS"
DEFAULT_CHUNKS = {"time": 1, "y": 256, "x": 256}
DEFAULT_COMPLETE_MARKER = ".monthly_complete.json"
...
def build_monthly_mosaic(
    *,
    ...
    with _open_zarr(input_path) as ds:
        ...
        # Monthly temporal composite keeps a true time axis while reducing
        # sparse per-acquisition coverage from instantaneous scenes.
        monthly = src_var.astype(np.float32).resample(time=freq).mean(
            skipna=True, keep_attrs=True
        )

        out_ds = monthly.to_dataset(name=variable)
        ...
        LOGGER.info(
            "Writing monthly mosaic %s with chunks=(%d,%d,%d)",
            output_path,
            chunks["time"],
            chunks["y"],
            chunks["x"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create monthly CHL mosaic Zarr from the raw CHL mosaic Zarr."
    )
    ...
    parser.add_argument(
        ...
        help="Output monthly CHL mosaic Zarr.",
    )
    parser.add_argument("--variable", default=DEFAULT_VARIABLE)
    parser.add_argument("--freq", default=DEFAULT_FREQ, help="Resample frequency (default: MS).")
    parser.add_argument(
        "--chunks",
